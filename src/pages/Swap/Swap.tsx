...
function Swap({
  ...
  const debouncedTokenOneAmountInput: string = useDebounce(
    tokenOneAmountInput,
    650
  );
  const debouncedTokenTwoAmountInput: string = useDebounce(
    tokenTwoAmountInput,
    650
  );
  ...
  const [isRefreshAnimationActive, setIsRefreshAnimationActive] =
    useState(false);
  ...
  const [hasUnlimitedAssociations, setHasUnlimitedAssociations] =
  ...
  const smartNodeSocket = async () => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        showLoader();
        let randomNode =
          HSUITE_NODES[Math.floor(Math.random() * HSUITE_NODES.length)];
        let nodeSocket: any = new SmartNodeSocket(
          randomNode,
          wallet.address,
          providers.HSuite.getApiKey()
        );

        nodeSocket.getSocket("gateway").on("connect", async () => {
          console.log(
            `account ${wallet.address} connected to node ${
              nodeSocket.getNode().operator
            }`
          );
        });
        ...
        nodeSocket.getSocket("gateway").on("disconnect", async () => {
          console.log(
            `account ${wallet.address} disconnected from node ${
              nodeSocket.getNode().operator
            }`
          );
        });
        ...
        nodeSocket
          .getSocket("gateway")
          .on("authenticate", async (event: any) => {
            if (event.isValidSignature) {
              resolve({
                message: `account ${wallet.address} authenticated to node ${
                  nodeSocket.getNode().operator
                }, ready to operate with websockets/write operations...`,
                socket: nodeSocket,
              });
            } else {
              showToast(
                "Connection issue",
                `Account ${wallet.address} can't connect to node ${
                  nodeSocket.getNode().operator
                }`,
                toastTypes.error
              );
        ...
        nodeSocket
          .getSocket("gateway")
          .on("authentication", async (event: any) => {
            let payload = {
  ...
  const changeAmountOne = (value: string) => {
    ...
    if (value.match(/^[0-9]{0,10}(?:\.[0-9]{0,8})?$/)) {
      setTokenOneAmountInput(
        trimNumberString(
          value
            ? [".", "0"].includes(value.charAt(value.length - 1))
              ? value
  ...
  const parseTokenOneAmount = (): BigNumber => {
    const trimedTokenOneAmount = trimNumberString(
      tokenOneAmount,
      tokenOne.decimals
    );
  ...
  const parseTokenTwoAmount = (): BigNumber => {
    const trimedTokenTwoAmount = trimNumberString(
      tokenTwoAmount,
      tokenTwo.decimals
    );
  ...
  const fetchDex = async () => {
    ...
    if (bestRate.aggregatorId === AggregatorId.HSuite) {
      ...
      socketConnection.socket
        .getSocket("gateway")
        .on("swapPoolRequest", async (resPool: any) => {
          try {
            if (resPool.status === "success") {
              let transaction: TransferTransaction = Transaction.fromBytes(
                new Uint8Array(resPool.payload.transaction)
              ) as TransferTransaction;
              const minToReceive = parseTokenTwoAmount()
                .mul(1000 - slippage * 10)
                .div(1000);
              let amountTo: BigNumber;
              if (tokenTwo.solidityAddress === ethers.constants.AddressZero) {
                amountTo = BigNumber.from(
              ...
              let signedTransactionBytes = await wallet.signTransaction(
                transaction
              );

              socketConnection.socket
                .getSocket("gateway")
                .on("swapPoolExecute", (responseEvent: any) => {
              ...
              socketConnection.socket.getSocket("gateway").emit(
                "swapPoolExecute",
                {
      ...
      if (tokenOne.solidityAddress === ethers.constants.AddressZero) {
        amountFromHsuite = amountFromHsuite
          .mul(1000 - providers[bestRate.aggregatorId].feePromille)
          .div(1000);
      } else if (tokenOne.symbol === "HSUITE") {
        const hSuiteFee = Math.max(
          10000,
          amountFromHsuite
            .mul(providers[bestRate.aggregatorId].feeDEXPromille)
            .div(1000)
            .toNumber()
        );
      ...
      let swapObj = {
        baseToken: {
          details: {
            id:
              tokenOne.solidityAddress === ethers.constants.AddressZero
                ? "HBAR"
                : tokenOne.address,
        ...
        swapToken: {
          details: {
            id:
              tokenTwo.solidityAddress === ethers.constants.AddressZero
                ? "HBAR"
                : tokenTwo.address,
      ...
      if (tokenOne.solidityAddress !== ethers.constants.AddressZero) {
        ...
        try {
          const approveTransaction = await wallet.executeTransaction(
            allowanceTx
          );
      ...
      if (bestRate.transactionType === TransactionType.SPLIT_SWAP) {
        ...
        if (tokenTwo.solidityAddress !== ethers.constants.AddressZero) {
          const [assoc1ToProvider, assoc2ToProvider] = await Promise.all([
            checkProviderAssociation(
              bestRate.aggregatorId[0],
              tokenTwo.address
            ),
            checkProviderAssociation(
              bestRate.aggregatorId[1],
              tokenTwo.address
            ),
          ]);
          if (!assoc1ToProvider) {
        ...
        swapTransaction = await new ContractExecuteTransaction()
          ...
          .setFunction(
            ...
            new ContractFunctionParameters()
              ...
              .addBytesArray(
                bestRate.path.map((path) => bytesFromHex(path.substring(2)))
              )
              .addUint256Array(
                // @ts-ignore
        ...
        let additionalAssociationGas = 0;
        if (tokenTwo.solidityAddress !== ethers.constants.AddressZero) {
          const assocToProvider = await checkProviderAssociation(
            bestRate.aggregatorId,
            tokenTwo.address
          );
          if (!assocToProvider) {
            additionalAssociationGas += ASSOCIATION_GAS;
        ...
        swapTransaction = await new ContractExecuteTransaction()
          ...
          .setPayableAmount(
            tokenOne.solidityAddress === ethers.constants.AddressZero
              ? feeOnTransfer
                ? ethers.utils.formatUnits(
                    bestRate.amountIn.mul(1000 + slippage * 10).div(1000),
                    8
                  )
      ...
      if (execTransaction.res?.transactionId) {
        const idTransaction = `${execTransaction.res.transactionId
          .toString()
          .substr(0, 4)}${execTransaction.res.transactionId
          .toString()
          .substr(4)
          .replace(/@/, "-")
          .replace(".", "-")}`;
        setTimeout(() => {
          axios
            .get(`${MIRRORNODE}/api/v1/transactions/${idTransaction}`)
            .then((res) => {
              if (res?.data?.transactions?.[0]?.result) {
                ...
                showToast(
                  "Transaction",
                  "Error on processing transaction",
                  toastTypes.error
                );
              }
            })
  ...
  const getActionButton = () => {
    ...
    if (wallet.associatedTokens && tokenOne && tokenTwo) {
      if (
        !(
          wallet.associatedTokens.has(tokenOne.address) ||
          tokenOne.symbol === typeWallet.HBAR
        ) ||
        !(
          wallet.associatedTokens.has(tokenTwo.address) ||
          tokenTwo.symbol === typeWallet.HBAR
        )
      ) {
        allTokensAssociated = false;
      }
      const isHSuiteRequired =
        sortedPrices?.length > 0
          ? sortedPrices[activeRoute].aggregatorId === AggregatorId.HSuite
          : false;
      const hSuiteToken = tokens.find(
        (token) => token.solidityAddress === HSUITE_TOKEN_ADDRESS
      );
      if (
        isHSuiteRequired &&
        !wallet.associatedTokens?.has(hSuiteToken?.address)
      ) {
        allTokensAssociated = false;
      }
    }
    if (!allTokensAssociated && !hasUnlimitedAssociations) {
      return (
        <AssociateNewToken
          handleClick={associateToken}
          associatedButtons={associatedButtons}
        />
      );
    }
    ...
    return (
      <button className="button swap__button" onClick={fetchDex}>
        Swap
      </button>
    );
  };
  ...
  const fetchPrices = async (): Promise<void> => {
    ...
    try {
      ...
      const amount = feeOnTransfer
        ? parseTokenTwoAmount().toString()
        : parseTokenOneAmount().toString();
      const req = `${API}/rates?tokenFrom=${tokenOne.solidityAddress}&tokenTo=${
        tokenTwo.solidityAddress
      }&amount=${amount}&isReverse=${feeOnTransfer.toString()}`;
      const res = await axios.get(req);
      setSortedPrices(
        res.data.map((price: any) => ({
          ...price,
          amountIn: Array.isArray(price.amountFrom)
            ? price.amountFrom.map((amount: string) => BigNumber.from(amount))
            : BigNumber.from(price.amountFrom),
          amountOut: Array.isArray(price.amountTo)
            ? price.amountTo.map((amount: string) => BigNumber.from(amount))
            : BigNumber.from(price.amountTo),
        }))
      );
    } catch (e) {
      console.log(e);
  ...
  const associateToken = async (token: Token) => {
    ...
    if (result) {
      if (result.error) {
        ...
        showToast(
          "Associate Token",
          `Token ${token.name} associated to account`,
          toastTypes.success
        );
      }
    } else {
  ...
  const checkProviderAssociation = async (
    aggregatorId: AggregatorId,
    tokenId: string
  ) => {
    try {
      const tokenInfo = await axios.get(
        `${MIRRORNODE}/api/v1/accounts/${PROVIDER_ADDRESSES[aggregatorId]}/tokens?token.id=${tokenId}`
      );
  ...
  const checkUnlimitedAssociations = async () => {
    ...
    try {
      const accountInfo = await axios.get(
        `${MIRRORNODE}/api/v1/accounts/${wallet.address}`
      );
      setHasUnlimitedAssociations(
        accountInfo.data?.max_automatic_token_associations === -1
      );
    } catch (e) {
      console.error(e);
    }
  ...
  const checkAssociateTokens = () => {
    ...
    if (
      ...
      const isHSuiteInList = !!tokensToAssociate.find(
        (token) => token.solidityAddress === HSUITE_TOKEN_ADDRESS
      );
  ...
  const getSwapRouteCoin = (
    solidityAddress: string
  ): ReactElement | undefined => {
    const token = tokens.find(
      (token) => token.solidityAddress === solidityAddress
    );
    if (!token) {
      return;
    }
    return (
      <div className="swap__route-coin" key={solidityAddress}>
        <img
          src={token.icon}
          alt={token.symbol}
  ...
  const getTokenOneAmount = (): string => {
    if (tokenOne) {
      return wallet.associatedTokens
        ?.get(tokenOne.address || "HBAR")
        ?.div(Math.pow(10, tokenOne.decimals))
        .toString();
    }
    return "";
  };
  ...
  const getTokenTwoAmount = (): string => {
    if (tokenTwo) {
      return wallet.associatedTokens
        ?.get(tokenTwo.address || "HBAR")
        ?.div(Math.pow(10, tokenTwo.decimals))
        .toString();
    }
    return "";
  };
    ...
    if (
      ...
      fetchPrices().then(() => {
        ...
        refreshTimer.current = setTimeout(
          refreshRate,
          25000 + 30 * refreshCount.current * refreshCount.current
        );
      });
    }
  };
  ...
  const setTargetPrice = (route: number) => {
    if (feeOnTransfer) {
      const bestAmountIn = summAmount(
        sortedPrices?.[route]?.amountIn
      )?.toString();
      if (tokenTwoAmount && bestAmountIn) {
        setTokenOneAmountInput(
          ethers.utils.formatUnits(bestAmountIn, tokenOne?.decimals)
        );
      } else {
        setTokenOneAmountInput("0");
      }
    } else {
      const bestAmountOut = summAmount(
        sortedPrices?.[route]?.amountOut
      )?.toString();
      if (tokenOneAmount && bestAmountOut) {
        setTokenTwoAmountInput(
          ethers.utils.formatUnits(bestAmountOut, tokenTwo?.decimals)
        );
      } else {
        setTokenTwoAmountInput("0");
      }
  ...
  useEffect(() => {
    ...
    if (searchParams.get("tokenFrom")) {
      const tokenFromQuery = tokens.find(
        (token) => token.solidityAddress === searchParams.get("tokenFrom")
      );
      if (tokenFromQuery) {
    ...
    if (searchParams.get("tokenTo")) {
      const tokenToQuery = tokens.find(
        (token) => token.solidityAddress === searchParams.get("tokenTo")
      );
      if (tokenToQuery) {
      ...
      <div className="swap">
        ...
        <div className="swap__field-wrapper">
          <div className="swap__field">
            ...
            <div className="swap__chip-wrapper">
              <button
                className="button swap__chip"
                onClick={() => setInputByDivider(4)}
              >
                25%
              </button>
              <button
                className="button swap__chip"
                onClick={() => setInputByDivider(2)}
              >
                50%
              </button>
              <button
                className="button swap__chip"
                onClick={() => setInputByDivider(1)}
              >
                MAX
              </button>
            </div>
            <div className="swap__coin" onClick={() => openTokensModal(1)}>
              ...
              />
              <span className="swap__coin-name text--gradient">
                {tokenOne?.symbol}
              </span>
            </div>
            {getTokenOneAmount() ? (
              <span className="swap__balance">
                Balance: {getTokenOneAmount()}
              </span>
            ) : (
              ""
            )}
          ...
          <button className="swap__switch" onClick={switchTokens}>
            &#10607;
          </button>
          <div className="swap__field">
            <input
        ...
        <div className="swap__details">
          {tokenOneAmount !== "0" && tokenTwoAmount !== "0" ? (
            <div className="swap__min-receive">
              ...
              <Popover
                ...
                content={
                  feeOnTransfer
                    ? `End price is an estimate. You will sell at most ${ethers.utils.formatUnits(
                        parseTokenOneAmount()
                          .mul(1000 + slippage * 10)
                          .div(1000)
                          .toString(),
                        ...
                        tokenOne?.symbol
                      } or the transaction will be aborted.`
                    : `End price is an estimate. You will receive at least ${ethers.utils.formatUnits(
                        parseTokenTwoAmount()
                          .mul(1000 - slippage * 10)
                          .div(1000)
                          .toString(),
          ...
          <div className="swap__slippage">
            <div className="swap__slippage-title">
              Slippage Tolerance&nbsp;
              <Popover
                trigger="hover"
                placement="right"
                content="If prices will change on more than provided percent during the swap - the transaction will be reverted."
              >
                <span className="swap__tooltip">
                  <svg className="swap__question" viewBox="0 0 20 20">
                    <use
            ...
            <div className="swap__slippage-options">
              <Radio.Group
                onChange={handleSlippage}
                value={slippage}
                className="swap__radio-group"
              >
        ...
        <div className="swap__routes">
          <div
            className={`swap__routes-title ${
              checkAllRatesOpen ? "swap__routes-title--active" : ""
            }`}
            onClick={() => switchAllRates()}
          >
            Select route:
          </div>
          {checkAllRatesOpen
            ? sortedPrices.map(({ transactionType, aggregatorId, route }, i) =>
                transactionType === TransactionType.SWAP ? (
                  <div
                    className={`swap__route ${
                      activeRoute === i ? "swap__route--active" : ""
                    }`}
                    key={i}
                    ...
                    <div className="swap__route-stages">
                      {aggregatorId.map((id, i) => (
                        <div className="swap__route-split" key={id + i}>
                          ...
                          {route[i]
                            .slice(0, -1)
                            .map((coin) => getSwapRouteCoin(coin))}
                        </div>
                      ))}
                    </div>
          ...
          <div className="swap__refresh">
            <div
              className={`swap__refresh-ticker${
                isRefreshAnimationActive ? " ticker--active" : ""
              }`}
              style={{
                animationDuration:
                  parseInt(
                    String(
                      (25000 +
                        30 * refreshCount.current * refreshCount.current) /
                        1000
                    )
                  ) + "s",
              }}
          ...
          {TOKEN_WITH_CUSTOM_FEES_LIST.includes(tokenOne?.solidityAddress) ||
          TOKEN_WITH_CUSTOM_FEES_LIST.includes(tokenTwo?.solidityAddress) ? (
            <div className="swap__warning">
              <span className="swap__warning-icon">&#9432;</span> You're
              exchanging tokens, which has custom fees on token side.
              Transaction may fail or it can affect output amount.
            </div>
          ) : (
            ""
          )}
